SELECT
    ...
    COALESCE(agg.`Usia 16-19 (L)`, 0) AS `Usia 16-19 (L)`,
    COALESCE(agg.`Usia 20-25 (L)`, 0) AS `Usia 20-25 (L)`,
    COALESCE(agg.`Usia 26-30 (L)`, 0) AS `Usia 26-30 (L)`,
    COALESCE(agg.`Usia 31-35 (L)`, 0) AS `Usia 31-35 (L)`,
    COALESCE(agg.`Usia 36+ (L)`, 0) AS `Usia 36+ (L)`,
    COALESCE(agg.`Total (L)`, 0) AS `Total (L)`
FROM (
...
SELECT
    ...
    SUM(
        COALESCE(agg.`Usia 16-19 (L)`, 0)
    ) AS `Usia 16-19 (L)`,
    SUM(
        COALESCE(agg.`Usia 20-25 (L)`, 0)
    ) AS `Usia 20-25 (L)`,
    SUM(
        COALESCE(agg.`Usia 26-30 (L)`, 0)
    ) AS `Usia 26-30 (L)`,
    SUM(
        COALESCE(agg.`Usia 31-35 (L)`, 0)
    ) AS `Usia 31-35 (L)`,
    SUM(
        COALESCE(agg.`Usia 36+ (L)`, 0)
    ) AS `Usia 36+ (L)`,
    SUM(COALESCE(agg.`Total (L)`, 0)) AS `Total (L)`
FROM (
        SELECT '9' AS id, 'Perselisihan Terus Menerus' AS FaktorPerceraian
        UNION ALL
        SELECT '10', 'Kawin Paksa'
